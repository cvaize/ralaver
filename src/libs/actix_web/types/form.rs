...
use actix_web::{
    body::EitherBody, error::UrlencodedError, FromRequest, http::header::CONTENT_LENGTH,
...
impl<T: Serialize> Responder for Form<T> {
    ...
    fn respond_to(self, _: &HttpRequest) -> HttpResponse<Self::Body> {
        match serde_qs::to_string(&self.0) {
            ...
            Err(err) => {
                let err = serde_qs_to_serde_urlencoded_error(err);
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        self.fut = Some(
            async move {
                ...
                if encoding == UTF_8 {
                    config.deserialize_bytes::<T>(&body).map_err(|e| {
                        UrlencodedError::Parse(serde::de::Error::custom(e))
                    })

                    // serde_qs::from_bytes::<T>(&body).map_err(|e| {
                    //     UrlencodedError::Parse(serde::de::Error::custom(e))
                    // })
                } else {
                    let body = encoding
                    ...
                    config.deserialize_str::<T>(&body).map_err(|e| {
                        UrlencodedError::Parse(serde::de::Error::custom(e))
                    })

                    // serde_qs::from_str::<T>(&body).map_err(|e| {
                    //     UrlencodedError::Parse(serde::de::Error::custom(e))
                    // })
                }
            }
...
fn serde_qs_to_serde_urlencoded_error(e: serde_qs::Error) -> serde_urlencoded::ser::Error{
    match &e {
        serde_qs::Error::FromUtf8(from) => serde_urlencoded::ser::Error::Utf8(from.utf8_error()),
        _ => serde_urlencoded::ser::Error::Custom(format!("{}", e).into())
    }
}
