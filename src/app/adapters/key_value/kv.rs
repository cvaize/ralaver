...
pub struct KVRepositoryKeyValueAdapter<'a> {
    pub repository: KVBucketRepository<'a>,
}

impl<'a> KVRepositoryKeyValueAdapter<'a> {
    pub fn new(repository: KVBucketRepository<'a>) -> Self {
        Self { repository }
    }

    /// Clearing expired values
    pub fn clean_expired_values(&self) -> Result<(), AppError> {
        let repository: &KVBucketRepository = &self.repository;
...
impl<'a> KeyValueRepository for KVRepositoryKeyValueAdapter<'a> {
    ...
    fn set<V: BytesValue>(&self, key: &str, value: V) -> Result<(), AppError> {
        ...
        self.repository.set(key.as_bytes(), &value)?;
        Ok(())
    }

    fn del(&self, key: &str) -> Result<(), AppError> {
        self.repository.remove(key.as_bytes())?;
        Ok(())
    }
}
...
impl<'a> IncrementableKeyValueRepository for KVRepositoryKeyValueAdapter<'a> {
    fn incr(&self, key: &str, delta: i64) -> Result<i64, AppError> {
        let mut value_with_timestamp: (i64, u64) =
            self.get_with_timestamp(key)?.unwrap_or((0, u64::MAX));
...
mod tests {
    use super::*;
    use crate::{make_config, KVRepository};
    ...
    fn make_adapter<'a>(name: &str) -> TestAdapterData<'a> {
        ...
        let kv_adapter = KVRepositoryKeyValueAdapter::new(kv_bucket_repository);
        TestAdapterData {
            kv_repository,
            kv_adapter,
        }
    ...
    fn test_get_del_set() {
        ...
        data.kv_adapter.repository.clear().unwrap();
