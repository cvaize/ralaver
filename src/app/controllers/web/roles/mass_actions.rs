...
const RL_TTL: u64 = 60;
const RL_KEY: &'static str = "roles_mass_actions";
        ...
        if data.action.is_some() && data.selected.is_some() {
            ...
            if ids.len() > 0 {
                if action.eq("delete") {
                    ...
                    role_service.delete_by_ids_throw_http(ids)?;
                    alert_variants.push(AlertVariant::RolesMassDeleteSuccess(join_vec(ids, ", ")));
                }
            }
        }
    } else {
